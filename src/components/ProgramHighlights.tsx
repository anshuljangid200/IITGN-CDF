...
const ProgramHighlights = () => {
  ...
  const activeProgram = useMemo(
    () => programs.find((program) => program.value === activeTab) ?? programs[0],
    [activeTab]
  );
    ...
    >
      <div className="container mx-auto px-4 lg:px-6">
        <div className="text-center max-w-4xl mx-auto mb-10 lg:mb-12 space-y-4">
          ...
          >
            Futurense IITGN PG Diploma Tracks
          </h2>
          <p className="text-muted-foreground text-base lg:text-lg leading-relaxed">
            Explore the residential GenAI-powered diplomas. Select a tab to preview the skills, hours, and certificate snapshot for each track.
          </p>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList
            ...
            className="sticky top-20 z-10 flex flex-wrap gap-3 rounded-2xl bg-muted/80 p-3 backdrop-blur"
          >
            {programs.map((program) => (
              <TabsTrigger
                ...
                value={program.value}
                className={cn(
                  "flex flex-col items-start rounded-2xl border px-4 py-3 text-left text-sm font-semibold transition-all",
                  "data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-soft data-[state=active]:border-primary/40",
              ...
              >
                <span className="leading-snug">{program.title}</span>
                <span className="text-xs font-normal text-muted-foreground/80 mt-1">{program.badge}</span>
              </TabsTrigger>
            ))}
          </TabsList>

          <TabsContent value={activeProgram.value}>
            <article className="rounded-[2rem] border border-border/70 bg-card/95 shadow-large p-6 lg:p-10">
              <div className="grid gap-8 lg:gap-10 lg:grid-cols-2">
                <div className="space-y-6">
                  ...
                  <dl className="grid gap-4 sm:grid-cols-3">
                    {activeProgram.stats.map((stat) => (
                      <div
                        key={`${activeProgram.value}-${stat.label}`}
                  ...
                  <ul className="space-y-4" aria-label={`${activeProgram.title} skill outcomes`}>
                ...
                <figure className="relative overflow-hidden rounded-[1.75rem] border border-border/70 bg-muted/20 shadow-soft min-h-[320px]">
                  <img
                    src={activeProgram.banner}
                    alt={`${activeProgram.title} certificate preview`}
                    loading="lazy"
