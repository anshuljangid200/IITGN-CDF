      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          {list.map((m, index) => (
            <div
              key={m.id}
              className={`group bg-card shadow-soft hover:shadow-medium p-6 rounded-2xl animate-fade-in ${styles[`delay${index * 100}`]}`}
            >
              ...
              <div className="text-center mt-2">
                <h3 className="text-lg md:text-xl font-semibold text-foreground mb-1 flex items-center justify-center gap-2">
                  <span>{m.name}</span>
                  {m.linkedin && (
                    <a href={m.linkedin} target="_blank" rel="noopener noreferrer" aria-label={`Visit ${m.name} on LinkedIn`} className="w-6 h-6 rounded-full bg-muted hover:bg-primary hover:text-primary-foreground flex items-center justify-center linkedin-icon-hover">
